...
VDPAUContext::~VDPAUContext(){
	...
	for(int i=0;i<m_usedQueue.size();++i){
        vdp_video_surface_destroy(m_usedQueue[i]->state->surface);
		delete m_usedQueue[i]->state;
        free(m_usedQueue[i]);
...
vdpau_render_state* VDPAUContext::obtain_free_render_state(){
  ...
  //  printf("obtain render\n");
	...
	else
	{
		...
		//s= m_usedQueue.front();
		vdp_video_state* pstate =  m_usedQueue.front();
		s = pstate->state;
...
int VDPAUContext::Push_render2que(AVFrame* frame,unsigned long upts)
{

	
		vdpau_render_state*rs=(vdpau_render_state*)frame->data[0];
		vdp_video_state *pstate = (vdp_video_state*)malloc(sizeof(vdp_video_state));
		pstate->state = rs;
		pstate->videopts = upts;
		
		//fprintf(stderr,"Frame <%d> Got..\n",i);
		//push in use que
		pthread_mutex_lock(&m_locker);
		m_usedQueue.push_back(pstate);
		//fprintf(stderr,"---pictype %d ,usedQue size=%d pts=%ld \n",frame->pict_type,m_usedQueue.size(),upts);
		pthread_mutex_unlock(&m_locker);
...
*/
int VDPAUContext::get_surface_data(AVPicture* pic,unsigned long *pUpts)
{
	int iret = -1;
	pthread_mutex_lock(&m_locker);
	...
	{
		vdp_video_state* pstate =  m_usedQueue.front();
		vdpau_render_state * pRenderState = pstate->state;
		
	    
		
		fetch_yuv(pRenderState,pic);
		m_usedQueue.pop_front();
		*pUpts = pstate->videopts;
		//printf(" get surface data from que size =%d pts=%ld\n",m_usedQueue.size(),pstate->videopts);
