...
void* TSDecoder_Instance::get_data_threadFun(void *param)
{
	...
	do{
		...
		gettimeofday(&tv1,NULL);
		start_time = tv1.tv_sec*1000+tv1.tv_usec/1000;
		if(vd_ctx->get_surface_data(&pic) >=0 )
		{
			videopts = start_time;
		...
		if(iret>0)
		{
			...
			if(vd_ctx->m_usedQueue.size() > SURFACE_QUE_SIZE-5){
				iret = iret/2;
				fprintf(stderr,"need accelet time surface size=%d \n",vd_ctx->m_usedQueue.size());
			}
		//	else if(vd_ctx->m_usedQueue.size() < SURFACE_QUE_SIZE/4){
		//		iret = iret*8/5;
		//	}	
...
void* TSDecoder_Instance::Audio_output_data_threadFun(void *param)
{
	...
	do{
		...
		if(iret>0)
		{
			...
			if(this0->m_audioUsedQueue.size() > AudioQue_Size-5){
				iret = iret/2;
	...
	do
	{
		...
		if (iret >= 0) 
		{
			...
			if (pFmt->streams[pkt.stream_index]->codec->codec_type == AVMEDIA_TYPE_VIDEO) 
			{
                ...
                #endif   
					//printf("-----push video to que data dts =%ld pts=%ld\n",pkt.dts,pkt.pts);
					vd_ctx->Push_render2que(pframe);
/*					{
...
*/					
			...
			else if (pFmt->streams[pkt.stream_index]->codec->codec_type == AVMEDIA_TYPE_AUDIO) 
			{
				if(this0->m_bGetData == true) //只有视频解码成功了才写音频
				{
					...
					//printf("-----packet audio data dts =%ld pts=%ld\n",pkt.dts,pkt.pts);
...
//logfile
	...
	if(avformat_open_input(&pFmt,m_tsParam.strURL,NULL,NULL)!=0)
	{
